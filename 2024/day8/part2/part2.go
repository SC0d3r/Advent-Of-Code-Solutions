...
func main() {
	...
	log.Println("MHs before POP", MHs)
	populateLines(MHs, rows, cols)
	log.Println("MHs after POP", MHs)
	...
	// add all of the origins to valids
	for mhs, _ := range MHs {
		validAntinodesVecs[Vec{mhs.origin.x, mhs.origin.y}] = struct{}{}
	}
...
func populateLines(MHs map[Loc]struct{}, rows int, cols int) {
	for loc, _ := range MHs {
		// 1. continue upward
		o := Vec{x: loc.origin.x, y: loc.origin.y}
		mh := loc.mh
		for {
			i := o.x + mh.x
			j := o.y + mh.y
			...
			// if o.x == 4 && o.y == 4 {
			log.Println("[up]", "o", o, "i", i, "j", j, "mh", mh)
			// }

			if i < 0 || i >= rows || j < 0 || j >= cols {
				// invalid
				if mh.x == 2 && mh.y == -1 {
					log.Println("[up breaking] i", i, "j", j, "mh", mh, "o", o)
				}
				break
			}
			// valid
		...
		for {
			...
			j := o.y - mh.y
			if o.x == 4 && o.y == 4 {
				log.Println("[down] i", i, "j", j, "mh", mh)
			}
			// i *= -idx
			// j *= -idx
			if i < 0 || i >= rows || j < 0 || j >= cols {
